...
In this document, we focus on how service accounts function—primarily from an application developer’s perspective
...
```bash
curl https://192.168.2.30:6443/api -k \
--header "Authorization: Bearer eyhgfgf..."
```
...
will reveal a volume mounted from a Secret (usually named something like `default-token-xxx`). You might see an excerpt similar to:
...
- **Automatic Token Mounting**: Pods can automatically mount the service account token at `/var/run/secrets/kubernetes.io/serviceaccount`, though this behaviour can be modified.
