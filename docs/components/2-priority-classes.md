...
![](../images/kubernetes-priorities-diagram.jpg)

> To check the current priority classes in your cluster, run the following commands: `kubectl get priorityclass`
...
> [!Caution]
> Note that only one priority class can be marked as the global default.
...
Consider a scenario where there are two workloads waiting to be sheduled: a critical application with a priority of 7 and a job with a priority of 5. With available resources, the higher priority critical application is scheduled first. If resources remain,
...
If you prefer that a higher priority Pod waits for resources rather than preempting lower priority Pods, set the `preemptionPolicy` to `Never`
