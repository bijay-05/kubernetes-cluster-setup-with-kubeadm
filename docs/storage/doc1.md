...
In this section of documents, we will explore essential storage concepts including persistent volumes (PV), persistent volume claims (PVC), and access modes. We will also learn how to configure your applications for persistent storage in Kubernetes.

Kubernetes provides a verstaile range of storage options that can adapt to different environments. This document specifically focuses on Kubernetes-native storage mechanisms, ensuring we understand the core concepts needed to integrate any third-party storage solutions with your cluster.

## Storage in Docker

In this part, we will explore how Docker handles storage drivers, manages data on the host filesystem, and implements a layered architecture to build images and run containers efficiently.

When Docker is installed, it creates a folder structure at `/var/lib/docker`, containing subdirectories such as `overlay2`, `containers`, `images`, and `volumes`. These directories store Docker images, container runtime data, and volumes. For example, files associated with running containers reside in the `container` directory, image files are stored under `images` and any created volumes are kept in the `volumes` directory.

## Docker Image Layers

Docker images are built using a layered architecture. Each instruction in a Dockerfile generates a new layer, container only the modifications from the previous layer. Consider this Dockerfile:
...
```bash
docker build Dockerfile -t appdev/custom-app
```

The layers are created in the following order:

1. The base Ubuntu image (approx 120 MB)
...
```bash
docker build Dockerfile2 -t appdev/second-app
```
...
> [!Important]
> When application code changes (for example, modifying `app.py`), Docker leverages the cache for all unchanged layers and rebuilds only the layer (and layers after that) with the new code
...
The `--mount` flag provides a more explicit syntax by requiring all parameters to be speicified. The following command is equivalent to the bind mount example above:
